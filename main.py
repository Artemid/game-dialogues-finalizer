...
def load_from_xml(inp_filename, collection):
	...
	local_path = os.path.dirname(inp_filename)
	...
	if handler.linked_files:
		for fName in handler.linked_files:
			xml_file = os.path.join(local_path, fName)
			if os.path.exists(xml_file):
				print("Processing linked file: %s" % xml_file)
				linked_handler = TEXTHandler(collection)
				parser.setContentHandler(linked_handler)
				parser.parse(xml_file)
			else:
				print("File does not exists: %s" % xml_file)


def main():
	print("Application started")

	os.chdir("C:\\projects\\PAPG\\build\\papgG5")
	print("Current dir: %s" % os.getcwd())

	xml_text = os.path.join("cdimage\\data", "translator_en.xml")

	collection = Collection()
	load_from_xml(xml_text, collection)
